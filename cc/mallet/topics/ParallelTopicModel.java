...
import Views.TopicModeling;
import Views.generateTopics;
import Views.processAutoDetectNumberOfTopics;
...
public class ParallelTopicModel implements Serializable {
	...
	public void estimate () throws IOException {
		...
		for (int iteration = 1; iteration <= numIterations; iteration++) {
			...
			double percent;
			if (TopicModeling.process_GenerateTopics_running) {
				TopicModeling.progressBarMallet.setValue(generateTopics.count);
				...
				TopicModeling.progressBarMallet.setString(
						"Doing " + (generateTopics.count) + " / " + generateTopics.numIterations + " ( " + percent
								+ " % )  Time remaining: " + remainingTime / 60 + " m " + remainingTime % 60 + " s");
			}
			if (TopicModeling.ProcessAutoDetectNumberOfTopics_running) {
				TopicModeling.progressBarMallet.setValue(processAutoDetectNumberOfTopics.count);
			...
			if (TopicModeling.process_GenerateTopics_running && TopicModeling.process_GenerateTopics.isCancelled()) { 
				TopicModeling.Out("The process of generating topics had been canceled by user  \n"); 
			...
			if (TopicModeling.ProcessAutoDetectNumberOfTopics_running && TopicModeling.threadProcessAutoDetectNumberOfTopics.isCancelled()) { 
				TopicModeling.Out("The process of 'Auto Detect Number Of Topics' had been canceled by user  \n"); 
				TopicModeling.btnGetCompositionTable.setEnabled(true); 
				TopicModeling.btnEstimateTopics.setEnabled(true);
				TopicModeling.btnXCancel.setEnabled(false);
